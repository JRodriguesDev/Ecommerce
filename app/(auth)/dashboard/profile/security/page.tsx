...
const Security = async () => {
    ...
    return (
                        ...
                        <div className="p-6 flex items-center justify-between hover:bg-zinc-800/20 transition-all cursor-pointer group">
                            <div className="flex items-center gap-5">
                                <div className="p-2.5 bg-zinc-950 border border-zinc-800 rounded-lg text-zinc-600 group-hover:text-blue-500 group-hover:border-blue-500/20 transition-all">
                                    {<LuLock size={20} className={hasPassword ? "text-blue-500" : ""} />} 
                                </div>
                                ...
                                <div className="space-y-0.5">
                                    ...
                                    <p className="text-sm font-bold text-zinc-200 group-hover:text-white transition-colors">
                                        {hasPassword ? "••••••••••••" : "Senha não definida"}
                                    </p>
                            ...
                            <SecurityActionButton userId={session!.user!.id!} type="PASSWORD" label={hasPassword ? "ALTERAR" : "DEFINIR"}/>
                        </div>
        ...
        <SecurityActionButton type="2FA" label={isTwoFactorEnabled ? "DESATIVAR" : "ATIVAR"} userId={session!.user!.id!} isTwoFactorEnabled={isTwoFactorEnabled}/>
    </div>
                    </Card>
                </section>
                {/* NOTA DE RODAPÉ */}
